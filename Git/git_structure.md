...
파일 내용이 동일할 경우 다른 사용자가 했든  트리 객체의 해시내용이 같음 ​	
...
git add시 트리라는 파일의 업데이트 되지않고 인덱스 에먼저 업데이트 됨 
`$ git write -tree`사용시 index의 내용을 캡쳐해 트리 객체에 저장
## 팩 파일(Pack File)
Git에서 다른파일과 매우 유사한 파일을 찾아 파일의 내용을 따로 저장후, 유사한 다른 파일간의 차이점(**델타**)을 찾아서 그것만 따로 저장후 팩에 저장

**콘텐츠(파일 내용) 중심으로 이루어져서** 버전 혹은 파일명이 다르더라도 저장소 내에 있고 내용이 유사하면 패킹이 이루어짐
